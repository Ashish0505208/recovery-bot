...
async function startRecovery(){
    ...
    for(const userId of users){
        ...
        try{
            ...
            await user.send(
`⚠️ Community Recovery Notice(joined already? Then please ignore:-) )
...
${getBackupInvite()}`
            ...
            failedUsers.push({
            username:data[getGuildId()][userId].username,
            id:userId
            });
        }
    }

    const FAILED_FILE=path.join(dataDir,"failed_dms.txt");
...
client.on("messageCreate",async(message)=>{
    ...
    await message.channel.send({
        content:"Failed DM Report",
        files:[result.failedFile]
    });
